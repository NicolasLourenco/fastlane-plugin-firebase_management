  ...
  module Actions
    class FirebaseManagementUploadShaAction < Action
      def self.run(params)
        ...
        # login
        api = nil
        if !params[:service_account_json_path].nil?
          api = manager.serviceAccountLogin(params[:service_account_json_path])
        elsif !params[:email].nil? && !params[:client_secret_json_path].nil?
        ...
        [
          ...
          FastlaneCore::ConfigItem.new(key: :cert_type,
                                       ...
                                       verify_block: proc do |value|
                                                       types = %i[SHA_1 SHA_256]
                                                       unless types.include?(value.to_sym)
                                                         UI.user_error!("Type must be in #{types}")
                                                       end
                                                     end),
